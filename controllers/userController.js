        ...
        if(userSession.user_id){
            ...
            if(passwordvalid){
                if(newpassword == confirmpassword){
                    ...
                    await userData.save()
                    res.render('changepassword',{user:check,message:"Password updated successfully."})
                }else{
                    res.render('changepassword',{user:check,message:"New password and confirm password do not match."})
                }
             
            }else{
                res.render('changepassword',{user:check,message:'Current password is incorrect.'})
            }
...
const resetpassword=async(req,res)=>{
    try {
        ...
        const updatedata=await User.updateOne({mobile:number},{$set:{password:securePassword}})
        if(updatedata){
...
const loadshop=async(req,res)=>{
    try {
        ...
        res.render('shop',{products:productData,user:check,category:categorydata})
...
const updateshop=async(req,res)=>{
    try {
        ...
        if(price === 'High' && req.body.category){
            productdata=await Product.find({genre:{$in:categoryfilter}}).sort({price:-1})
        }else if(price ==='Low' && req.body.category){
            productdata=await Product.find({genre:{$in:categoryfilter}}).sort({price:1})
        }
        else if(price ==='lastest arrivals' && req.body.category){
            productdata=await Product.find({genre:{$in:categoryfilter}}).sort({$natural:-1})
...
const viewProductPage=async(req,res)=>{
    ...
    try {
        ...
        res.render('productDetails',{products:productData,user:check,productone:productdetails})
    } catch (error) {
        console.log(error.message);
    }
}
...
const loadcart=async(req,res)=>{
    try {
        ...
        if(userSession.user_id){
            ...
            res.render('cart',{user:check,id:userSession.user_id,cartProducts:completeUser.cart})
        }else{
            res.redirect('/login')
        }
    } catch (error) {
...
const addToCart=async(req,res)=>{
    try {
        ...
        if(userSession.user_id){
            ...
            // res.redirect('/login')
            res.json({success:false,message:"please login to add product to cart"})
        }
...
const userdashboard=async(req,res)=>{
    try {
        ...
        if(userSession.user_id){
            ...
            const orderData=await Orders.find({userId:userSession.user_id}).sort({OrderedAt:-1})
            res.render('dashboard',{user:check,useraddress:addressData,user:userData,userorders:orderData})
        }else{
            res.redirect('/login')
        }
...
const updateuser=async(req,res)=>{
    try {
       ...
       const updated= await User.findByIdAndUpdate({_id:id},{$set:{name:req.body.name,email:req.body.email,mobile:req.body.phone}})
        res.redirect('/dashboard')
    } catch (error) {
        console.log(error.message);
    }
    ...
    try {
        if(req.session.user1){check=true} else check=false
         ...
         if(userSession.user_id){
            ...
            res.render('checkout',{user:check,useraddress:addressData,selectedaddress:selectaddress,cartProducts:completeUser.cart,id:userSession.user_id,coupons:coupon})
        ...
        if(userSession.user_id){
            ...
            for(let key of userData.cart.item){
                for(let value of productData){
                    if(new String(value._id).trim() == new String(key.productId._id).trim()){
                        value.quantity = value.quantity -key.qty
                        await value.save()
                    }
                }
            }
            await Orders.updateOne({userId:userSession.user_id,_id:userSession.currentorder},{$set:{'status':'Bulid'}})
...
const vieworderdetails=async(req,res)=>{
    try {
        ...
        userSession=req.session
            ...
            res.render('vieworderdetails',{user:check,orderdetails:orderData,address:addressData,users:addressId})
...
const returnOrders=async(req,res)=>{
    const id=req.query.id
    await Orders.findByIdAndUpdate({_id:id},{$set:{'status':'Order Returned'}})
    res.redirect('/dashboard')
}
...
const updateaddress=async(req,res)=>{
    try {
        const id=req.body.id
        const updatedaddress=await Address.findByIdAndUpdate({_id:id},{$set:{firstname:req.body.firstname,lastname:req.body.lastname,
        country:req.body.country,
        address:req.body.address,
        city:req.body.city,
        state:req.body.state,
...
const editaddersscheckout=async(req,res)=>{
    try {
        ...
        res.render('editaddresscheckout',{user:check,useraddress:addressData})
    } catch (error) {
        console.log(error.message)
    }
}
...
const updateaddresscheckout=async(req,res)=>{
   try {
    ...
    const updatedaddresscheckout=await Address.findByIdAndUpdate({_id:id},{$set:{firstname:req.body.firstname,lastname:req.body.lastname,
    country:req.body.country,
    address:req.body.address,
    city:req.body.city,
    state:req.body.state,
...
const coupondata=await Coupon.findOne({$and:[{Couponname:req.body.couponname},{expiry_Data:{$gte:currentdate}}]})
res.send({coupondata})
}
